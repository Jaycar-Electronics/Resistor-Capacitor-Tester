...
void loop()
{
  ...
  {
    cdetect = detect();
  } //goto selection if made, otherwise autodetect
  switch (cdetect)
  {
  case 1:
    doresistor();
...
{
  while (1)
  { //do it all repeatedly till the resistor is disconnected (see return; below)
    int a1, a2, a3, a, diff;
    ...
    if (abs(MIDRANGE - a2) < diff)
    { //R2 is better
      a = a2;
      rdiv = R2VALUE;
      diff = abs(MIDRANGE - a2);
    }
    if (abs(MIDRANGE - a3) < diff)
    { //R3 is better
      a = a3;
      rdiv = R3VALUE;
    ...
    if (a >= 1022)
    { //open circuit, resistor has probably been detached, so go back to main screen, avoid div/0 error
      return;
    }
    else
    {
...
void docapacitor()
{
  ...
  while (1)
  { //stay in this loop until disconnected
    pinMode(R1PIN, OUTPUT);
    ...
    if (ahi > 1000)
    {                   //very high termination, probably get more accurate results with 2nd resistor
...
int ndig(float c)
{ //work out how many decimal places to show
  if (c > 100)
  {
    return 0;
  }
...
int detect()
{ //auto detect whether it's a cap or resistor connected
  int ahi, alo;
  lcd.setCursor(0, 1);
...
{ //helper function to print multiplier
  switch (mult)
  {
  case 1:
    lcd.print('R');
...
void dobuttons()
{ //updates variables. debounces by only sampling at intervals
  int key;
  ...
  {
    lcdsplash();
  } //update display to show setting
}
